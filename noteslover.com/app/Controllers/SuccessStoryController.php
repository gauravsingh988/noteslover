...
class SuccessStoryController extends BaseController
{
    ...
    {
        $this->storyModel = new SuccessStoryModel();
    }
    ...
    public function details($slug)
    {
        ...
        $storyModel = new SuccessStoryModel(); // renamed for clarity
        ...
        $data['related_stories'] = $storyModel->where('is_deleted', '0')
            ->where('status', 'active')
            ->where('id !=', $storyId)
            ->orderBy('id', 'DESC')
            ->findAll(6); // Limit to 6 related stories
